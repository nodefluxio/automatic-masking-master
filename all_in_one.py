        ...
        for f in tqdm(files):
            ...
            if is_image(image_path):
                ...
                for i in range(len(mask)):
                    try:
                        os.mkdir(write_path + "/" + split_path[0])
                    except:
                        print(split_path[0], ' was created')
                    w_path = (
                        write_path
                        + "/"
                        + split_path[0]
                    ...
                    img_ratio = w/h
                    out_h_size = 720
                    img_resize = (int(out_h_size*img_ratio), out_h_size)
            ...
            for f in files:
                ...
                if is_image(image_path):
                    ...
                    for i in range(len(mask)):
                        ...
                        # img_ratio = w/h
                        # out_h_size = 720
                        # img_resize = (out_h_size, int(out_h_size*img_ratio))
                        # img = cv2.resize(img, img_resize)
...
def resize_and_save(path, out_h_size=720):
    images = os.listdir(path)
    # read image
    try:
        os.mkdir(path + '_masked' + '/')
    except:
        print('folder ', path, '_masked was created')
    for filename in images:
        ...
        h, w, _ = img.shape
        img_ratio = w/h
        resize = (int(out_h_size*img_ratio), out_h_size)
        img = cv2.resize(img, resize)

        folder_out = path + '_masked' + '/' + filename.split('.')[0]
        try: 
            print(folder_out)
            os.mkdir(folder_out)
        except:
            print('was created: ', folder_out)
        filename_out = filename.split('.')[0] + '.jpg'
        output_filename = folder_out + '/' + filename_out
        
        cv2.imwrite(output_filename, img)
        print('Success resize image. Ouput: ', filename_out) 
   
def generate_csv(path_out):
    ...
    for foldername in folders:
        arr_idx.append(idx)
        arr_foldername.append(foldername)
        arr_status.append('nodeflux')
        idx += 1
    dict_csv = {
        'id': arr_idx,
        'username': arr_foldername,
        'status': arr_status,
        'foldername': arr_foldername,
    }
    df = pd.DataFrame(dict_csv)
    df.to_csv(path_out + '/' + "batch.csv", header=None, index=False)

if __name__ == "__main__":
    # Command-line input setup
    parser = argparse.ArgumentParser(description="MaskTheFace - Python code to mask faces dataset")
    parser.add_argument("--path", type=str, default="", help="Path to either the folder containing images or the image itself",)
    parser.add_argument("--mask_type", type=str, default="surgical", choices=["surgical", "N95", "KN95", "cloth", "gas", "inpaint", "random", "all"], help="Type of the mask to be applied. Available options: all, surgical_blue, surgical_green, N95, cloth", )
    ...
    args.verbose = True
    # resize and save 
    resize_and_save(path_in, out_h_size=720)
    # masking kn95
    masking(args)
    args.code = "surgical_green"
    # masking surgical green
    masking(args)
    # generate csv
    generate_csv(path_in + '_masked')
